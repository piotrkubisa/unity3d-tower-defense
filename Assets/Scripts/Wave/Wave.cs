    ...
    public bool Spawn()
    {
        ...
        if (canSpawn)
        {
            if (Time.time > lastSpawnTime + spawnDuration)
            {
                ...
                // just spawn it!
                GameObject go = Instantiate(prefab, spawnPoint.transform.position, spawnPoint.transform.rotation) as GameObject;
                EnemyBehaviourScript eb = go.gameObject.GetComponent<EnemyBehaviourScript>();
